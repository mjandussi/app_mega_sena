    ...
    if st.button("🚀 Gerar Análises", type="primary", use_container_width=True):
        with st.spinner("Processando dados e gerando análises..."):
            try:
                ...
                if numeros_input:
                    try:
                        # Processar os números digitados
                        ...
                        if numeros_validos:
                            ...
                            # Calcular estatísticas para cada número
                            for numero in sorted(numeros_validos):
                                st.markdown(f"### 🎯 Número **{numero}**")
                                
                                # Contar quantas vezes o número apareceu
                                ...
                                # Gráfico de aparições ao longo do tempo
                                aparicoes_tempo = []
                                janela = 50  # Janela móvel de 50 sorteios
                                ...
                                if len(aparicoes_tempo) > 0:
                                    ...
                                    ax.plot(range(janela, len(df)+1), aparicoes_tempo, 
                                           color='steelblue', linewidth=2, alpha=0.7)
                                    ax.fill_between(range(janela, len(df)+1), aparicoes_tempo, 
                                                   alpha=0.3, color='steelblue')
                                    ax.set_xlabel('Concurso', fontsize=10, fontweight='bold')
                                    ax.set_ylabel(f'Aparições (janela de {janela})', fontsize=10, fontweight='bold')
                                    ax.set_title(f'Frequência do Número {numero} ao Longo do Tempo', 
                            ...
                            # Análise combinada - quantas vezes os números apareceram juntos
                            if len(numeros_validos) >= 2:
                                st.markdown("### 🔗 Análise Combinada")
                                st.write(f"Verificando quantas vezes os números {', '.join(map(str, sorted(numeros_validos)))} apareceram **juntos** no mesmo sorteio:")
